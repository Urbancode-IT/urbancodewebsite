    ...
    if ($action == 'enroll') {
        ...
        if ($conn->query($sql) === TRUE) {
            ...
            if ($name != 'Brochure') {
                ...
                if (!$userSuccess) {
                    echo "<div class='alert alert-danger'>Error: Failed to send confirmation email.</div>";
                }
            }

            // Admin email
            $reg = ($name == 'Brochure') ? '' : '<p style=" font-size: 16px;font-weight: 600;margin: auto;margin-bottom:10px;">New User Registration.</p>';
            $content = ($name == 'Brochure') ? 'A New User Downloaded Brochure with the Email: '.$email.'. and Phone: '.$mobile
                                              : 'A new user '.$name.', has registered with the Email: '.$email.'. and Phone: '.$mobile;

            $adminEmail = "info@urbancode.in";
            $adminSubject = "New User Registration";
            $adminheaders = "From: Admin@urbancode.in\r\n";
            ...
            if (!$adminSuccess) {
                echo "<div class='alert alert-danger'>Error: Failed to send notification to admin.</div>";
            }
        ...
        $phone = $_POST["phone"];
        $upi = $_POST["upi"];
        $query = "SELECT uc_id FROM users ORDER BY id DESC LIMIT 1";
        $resultold = $conn->query($query);
        ...
        if ($resultold) {
            ...
            $uc_id = generateUniqueID($lastUCID);
        }
        ...
        if (password_verify($repeatPassword, $password)) {
            ...
            if ($result->num_rows == 0) {
                ...
                $sql = "INSERT INTO users (uc_id, username, password, role, email, phone, upi) 
                        VALUES ('$uc_id', '$username', '$password', 'user', '$email', '$phone', '$upi')";
    
                if ($conn->query($sql) === TRUE) {
                    echo "Thanks for being a part of Our referral program. Your referral ID is <b>" . $uc_id . "</b>";
                } else {
        ...
        $sql = "SELECT id, password, role FROM users WHERE email='$username'";
        ...
        $sql = "INSERT INTO referrals (name, email, phone, location, course, comments, uc_id, status, payment, referral_fee) VALUES ('$name', '$email', '$phone', '$location', '$course', '$comments', '$uc_id', '$status', '$payment', '$referral_fee')";

        if ($conn->query($sql) === TRUE) {
            echo "Referred successfully";        
        }
    }else if($action == 'paymentForm'){
        $amount = $_POST["amount"];
        $status = $_POST["status"];
        $ref_fee = $_POST["ref_fee"];
        $ref_id = $_POST["ref_id"];        

        $sql = "UPDATE referrals SET payment=?, status=?, referral_fee=? WHERE id=$ref_id";

        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sss", $amount, $status, $ref_fee);
...
function generateUniqueID($id) {
    $currentID = $id;
    $currentID++;
    $paddedID = str_pad($currentID, 4, '0', STR_PAD_LEFT);
    return $paddedID;
}